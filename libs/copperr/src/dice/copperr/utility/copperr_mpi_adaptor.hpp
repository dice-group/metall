...
#include <dice/copperr/utility/mpi.hpp>
#include <dice/copperr/utility/copperr_mpi_datastore.hpp>
...
class copperr_mpi_adaptor {
 public:
  ...
  copperr_mpi_adaptor(dice::copperr::open_only_t, const std::string &root_dir_prefix,
                     const MPI_Comm &comm = MPI_COMM_WORLD)
      : m_mpi_comm(comm),
        m_root_dir_prefix(root_dir_prefix),
        m_local_copperr_manager(nullptr) {
    if (!priv_verify_num_partitions(root_dir_prefix, comm)) {
      ::MPI_Abort(comm, -1);
    }
    m_local_copperr_manager = std::make_unique<manager_type>(
  ...
  /// \param root_dir_prefix A root directory path of a Metall datastore.
  /// \param comm A MPI communicator.
  copperr_mpi_adaptor(dice::copperr::open_read_only_t,
  ...
  copperr_mpi_adaptor(dice::copperr::create_only_t, const std::string &root_dir_prefix,
                     const MPI_Comm &comm = MPI_COMM_WORLD,
  ...
  ~copperr_mpi_adaptor() {
    m_local_copperr_manager.reset(nullptr);
  ...
  manager_type &get_local_manager() { return *m_local_copperr_manager; }

  /// \brief Returns the Metall manager object of the process.
  /// \return A reference to a Metall manager object.
  const manager_type &get_local_manager() const {
    return *m_local_copperr_manager;
  }
  ...
  bool snapshot(const std::string &destination_dir_path,
    ...
    return priv_global_and(
        m_local_copperr_manager->snapshot(
            ds::make_local_dir_path(destination_dir_path, rank).c_str()),
        m_mpi_comm);
  }
  ...
  static bool remove(const std::string &root_dir_prefix,
    ...
    if (!copperr::mtlldetail::file_exist(
    ...
    bool copperr_dir = true;
    if (!copperr::mtlldetail::file_exist(
            ds::make_root_dir_path(root_dir_prefix) + "/" +
            k_datastore_mark_file_name)) {
      copperr_dir = false;
    }
    if (!priv_global_and(copperr_dir, comm)) {
      if (rank == 0) {
    ...
    for (int i = 0; i < size; ++i) {
      if (i == rank) {
        if (dice::copperr::mtlldetail::file_exist(
                ds::make_root_dir_path(root_dir_prefix)) &&
            !copperr::mtlldetail::remove_file(
                ds::make_root_dir_path(root_dir_prefix))) {
 ...
 private:
  static constexpr const char *k_datastore_mark_file_name =
      "copperr_mpi_datastore";
  static constexpr const char *k_partition_size_file_name =
      "copperr_mpi_adaptor_partition_size";
  ...
  static void priv_setup_root_dir(const std::string &root_dir_prefix,
    ...
    for (int i = 0; i < size; ++i) {
      if (i == rank && !copperr::mtlldetail::directory_exist(root_dir_path)) {
        if (!copperr::mtlldetail::create_directory(root_dir_path)) {
        ...
        if (!copperr::mtlldetail::create_file(mark_file)) {
  ...
  std::unique_ptr<manager_type> m_local_copperr_manager;
};
