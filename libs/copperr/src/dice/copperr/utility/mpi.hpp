...
inline int determine_local_root(const MPI_Comm &comm) {
  const char *shm_name = "copperr_local_root";
  ::shm_unlink(shm_name);
  barrier(comm);
  ...
  if (shm_fd == -1) {
    ...
    if (!copperr::mtlldetail::extend_file_size(shm_fd, shm_size, false)) {
  ...
  if (!copperr::mtlldetail::munmap(shm_fd, ptr, shm_size, false)) {
    logger::out(logger::level::warning, __FILE__, __LINE__,
