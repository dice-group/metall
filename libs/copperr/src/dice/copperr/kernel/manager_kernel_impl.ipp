...
// -------------------- //
template <typename chnk_no, std::size_t chnk_sz>
bool manager_kernel<chnk_no, chnk_sz>::create(const std::filesystem::path &base_dir_path,
...
bool manager_kernel<chnk_no, chnk_sz>::open(
    const std::filesystem::path &base_dir_path, const size_type vm_reserve_size_request) {
...
bool manager_kernel<chnk_no, chnk_sz>::snapshot(
    const std::filesystem::path &destination_base_dir_path,
...
bool manager_kernel<chnk_no, chnk_sz>::copy(
    const std::filesystem::path &source_base_dir_path, const std::filesystem::path &destination_base_dir_path,
    const int num_max_copy_threads) {
...
template <typename chnk_no, std::size_t chnk_sz>
bool manager_kernel<chnk_no, chnk_sz>::remove(const std::filesystem::path &base_dir_path) {
...
template <typename chnk_no, std::size_t chnk_sz>
bool manager_kernel<chnk_no, chnk_sz>::consistent(const std::filesystem::path &dir_path) {
...
template <typename chnk_no, std::size_t chnk_sz>
std::string manager_kernel<chnk_no, chnk_sz>::get_uuid(const std::filesystem::path &dir_path) {
...
bool manager_kernel<chnk_no, chnk_sz>::get_description(
    const std::filesystem::path &base_dir_path, std::string *description) {
...
bool manager_kernel<chnk_no, chnk_sz>::priv_reserve_vm_region(
  ...
  if (!m_vm_region) {
    METALL_ERROR("Cannot reserve a VM region of {} bytes", nbytes);
    m_vm_region_size = 0;
    return false;
  }
...
bool manager_kernel<chnk_no, chnk_sz>::priv_release_vm_region() {
  if (!mdtl::munmap(m_vm_region, m_vm_region_size, false)) {
    METALL_WARN("Cannot release a VM region at {} of {} bytes", m_vm_region, m_vm_region_size);
    return false;
  }
  m_vm_region = nullptr;
  m_vm_region_size = 0;
...
T *manager_kernel<chnk_no, chnk_sz>::priv_generic_construct(
  ...
  if (length > static_cast<std::size_t>(-1) / table.size) {
    return nullptr;
  }
  ...
  std::unique_ptr<void, std::function<void(void *)>> ptr_holder(
      ptr, [this](void *const ptr) {
        try {
          ...
          METALL_ERROR("Exception was thrown when cleaning up an object");
        }
      });
...
bool manager_kernel<chnk_no, chnk_sz>::priv_open(
    const std::filesystem::path &base_dir_path, const bool read_only,
  ...
  if (!priv_check_version(*m_manager_metadata)) {
    METALL_ERROR("Invalid version — it was created by Metall v{} (currently using v{})",
                 to_version_string(priv_get_version(*m_manager_metadata)),
                 to_version_string(METALL_VERSION));
    return false;
  }
...
bool manager_kernel<chnk_no, chnk_sz>::priv_create(
    const std::filesystem::path &base_dir_path, const size_type vm_reserve_size) {
  if (!priv_validate_runtime_configuration()) {
    return false;
  }

  if (vm_reserve_size > k_max_segment_size) {
    METALL_ERROR("Too large VM region size was requested ({} bytes)", vm_reserve_size);
    return false;
  }
...
bool manager_kernel<chnk_no, chnk_sz>::priv_set_version(
  ...
  if (mdtl::ptree::count(*metadata_json, k_manager_metadata_key_for_version) >
      0) {
    METALL_ERROR("Version information already exist");
    return false;
  }
...
bool manager_kernel<chnk_no, chnk_sz>::priv_set_uuid(
  ...
  if (!uuid_ss) {
    METALL_ERROR("Failed to convert UUID to std::string");
    return false;
  }

  if (mdtl::ptree::count(*metadata_json, k_manager_metadata_key_for_uuid) > 0) {
    METALL_ERROR("UUID already exist");
    return false;
  }
