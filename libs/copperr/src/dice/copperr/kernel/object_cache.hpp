...
#include <dice/copperr/detail/proc.hpp>
#include <dice/copperr/detail/hash.hpp>
...
inline constexpr unsigned int comp_chunk_size(
  ...
  return std::max(std::min(static_cast<unsigned int>(4096 / object_size),
...
inline constexpr typename bin_no_manager::bin_no_type comp_max_bin_no(
  ...
  for (; b < bin_no_manager::num_small_bins(); ++b) {
    ...
    if (max_per_cpu_cache_size < min_required_cache_size) {
      if (b == 0) {
        METALL_ERROR("The request max per-CPU cache size is too small");
        return 0;
      }
      --b;
      break;
...
#error "METALL_MAX_PER_CPU_CACHE_SIZE=byte must be defined"
 ...
 private:
  ...
  bool priv_allocate_cache() {
    ...
    if (!m_cache) {
      METALL_ERROR("Failed to allocate memory for the cache");
      return false;
    }
