...
/// \tparam copperr_manager_kernel_type A manager kernel type.
/// \warning
...
template <typename T, typename copperr_manager_kernel_type>
class stl_allocator {
 public:
  // -------------------- //
  ...
  using pointer = typename std::pointer_traits<
      typename copperr_manager_kernel_type::void_pointer>::
  ...
  using manager_kernel_type = copperr_manager_kernel_type;
 ...
 private:
  ...
  pointer priv_allocate(const size_type n) const {
    ...
    if (!get_pointer_to_manager_kernel()) {
      METALL_ERROR("nullptr: cannot access to manager kernel");
      throw std::bad_alloc();
    }
