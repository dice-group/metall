...
inline bool remove_file(std::filesystem::path const &path) {
    ...
    !defined(METALL_DISABLE_CXX17_FILESYSTEM_LIB)
  std::error_code ec;
  [[maybe_unused]] const auto num_removed = std::filesystem::remove_all(path, ec);
  return !ec;
#else
...
inline bool copy_file_dense(std::filesystem::path const &source_path,
  ...
  try {
    if (!fs::copy_file(source_path, destination_path,
      ...
      METALL_ERROR("Failed copying file {} -> {}", source_path.c_str(), destination_path.c_str());
      success = false;
    }
  } catch (fs::filesystem_error &e) {
    METALL_ERROR("{}", e.what());
...
inline bool copy_files_in_directory_in_parallel_helper(
    ...
    const std::function<bool(std::filesystem::path const &, std::filesystem::path const &)>
        &copy_func) {
...
inline bool copy_files_in_directory_in_parallel(
  ...
  return copy_files_in_directory_in_parallel_helper(
      ...
      [&sparse_copy](std::filesystem::path const &src, std::filesystem::path const &dst) -> bool {
