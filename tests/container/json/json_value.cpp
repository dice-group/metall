...
TEST(JSONValueTest, Assign) {
  ...
  {
    val = static_cast<signed char>(-1);
    GTEST_ASSERT_TRUE(val.is_int64());
    GTEST_ASSERT_EQ(val.as_int64(), static_cast<signed char>(-1));
  }

  {
    val = -3;
    GTEST_ASSERT_TRUE(val.is_int64());
    GTEST_ASSERT_EQ(val.as_int64(), -3);
  }

  {
    val = -4l;
    GTEST_ASSERT_TRUE(val.is_int64());
    GTEST_ASSERT_EQ(val.as_int64(), -4l);
  }

  {
    val = -5ll;
    GTEST_ASSERT_TRUE(val.is_int64());
    GTEST_ASSERT_EQ(val.as_int64(), -5ll);
  }

  {
    val = static_cast<unsigned char>(1);
    GTEST_ASSERT_TRUE(val.is_uint64());
    GTEST_ASSERT_EQ(val.as_uint64(), static_cast<unsigned char>(1));
  }

  {
    val = static_cast<unsigned short>(2);
    GTEST_ASSERT_TRUE(val.is_uint64());
    GTEST_ASSERT_EQ(val.as_uint64(), static_cast<unsigned short>(2));
  }

  {
    val = 3u;
    GTEST_ASSERT_TRUE(val.is_uint64());
    GTEST_ASSERT_EQ(val.as_uint64(), 3u);
  }

  {
    val = 4ul;
    GTEST_ASSERT_TRUE(val.is_uint64());
    GTEST_ASSERT_EQ(val.as_uint64(), 4ul);
  }

  {
    val = 5ull;
    GTEST_ASSERT_TRUE(val.is_uint64());
    GTEST_ASSERT_EQ(val.as_uint64(), 5ul);
  }
...
TEST(JSONValueTest, Emplace) {
...

  {
    val.emplace_int64() = -1;
    GTEST_ASSERT_TRUE(val.is_int64());
    GTEST_ASSERT_EQ(val.as_int64(), -1);
  }
